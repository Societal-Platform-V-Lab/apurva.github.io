...
//       "Voice of <span class='green-text'>Communities</span>",
//     "Collaborative Conversations":
//       "Collaborative <span class='green-text'>Conversations</span>",
//     "Knowledge Curation": "Knowledge <span class='green-text'>Curation</span>",
//     "Strategic Assessment":
//       "Strategic <span class='green-text'>Assessment</span>",
//     "Ecosystem Connection":
//       "Ecosystem <span class='green-text'>Connections</span>",
//   };
...
//             <span class="queryable-text">${
//               greenWords[item.title] || item.title
//             }</span>
...
// Function to generate HTML for each item
function generateHTML(item, index, totalItems) {
  let greenWords = {
    "Queryable Wisdom": "Queryable <span class='green-text'>Wisdom</span>",
    ...
    "Ecosystem Connections": "Ecosystem <span class='green-text'>Connections</span>",
    // Removed "Ecosystem Connection" from greenWords
  };

  // Check if this is the last item in the array
  const isLastItem = index === totalItems - 1;

  // Determine if the item should have green-border class
  const hasGreenBorder = !greenWords[item.title];
  ...
  return `
    <div class="col-md-12">
      <div class="blog d-lg-none d-sm-block d-md-block ${hasGreenBorder ? '' : 'green-border'} ${isLastItem ? '' : 'border-bottom'}">
        ...
        <a href="${item.link}" class="newrxp">Explore More</a>
        ${isLastItem ? '' : '<br />'} <!-- Conditionally include line break -->
      </div>
    </div>
  `;
}
...
function renderHTML() {
  ...
  jsonData.forEach((item, index) => {
    const html = generateHTML(item, index, jsonData.length);
    row.insertAdjacentHTML("beforeend", html);
  });
}
