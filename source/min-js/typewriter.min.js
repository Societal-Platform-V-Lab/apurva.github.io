document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const typingElement = document.getElementById("typewriter-change-text");
    const texts = JSON.parse(typingElement.getAttribute("data-texts"));
    const textArray = Array.isArray(texts) ? texts : [texts];
    let currentIndex = 0;
    ...
    function typeText(text, callback) {
      ...
      function type() {
        if (i < text.length) {
          ...
          typingElement.innerHTML = htmlContent;
          setTimeout(type, 100); // Speed for each character typing
        } else if (callback) {
          setTimeout(callback, 2000); // Pause after the sentence is completed
        }
      }
    ...
    function startTypingAnimation() {
      typeText(textArray[currentIndex], function () {
        currentIndex = (currentIndex + 1) % textArray.length;
        startTypingAnimation(); // Start the next sentence after the current one finishes
      });
    }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const typingElement = document.getElementById("typechange-text");
    const texts = JSON.parse(typingElement.getAttribute("data-texts"));
    const textArray = Array.isArray(texts) ? texts : [texts];
    let currentIndex = 0;
